...
- Restarts the voxtype daemon and waybar when the config changes via `run_onchange_after_restart-voxtype.sh.tmpl` (embeds the config hash, so it re-runs whenever `config.toml` changes; skips if voxtype isn't installed).
